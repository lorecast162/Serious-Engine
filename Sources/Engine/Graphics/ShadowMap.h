...
public:
  ...
  unsigned long sm_ulFlags;                  // various flags
  ...
  unsigned long *sm_pulCachedShadowMap;   // pointer to cached calculated shadow map
  unsigned long *sm_pulDynamicShadowMap;  // pointer to shadow map with dynamic lights
  long sm_slMemoryUsed;          // memory in use by shadow map (in bytes)
  ...
  unsigned long sm_ulInternalFormat;    // last format in which shadowmap was uploaded
  unsigned long sm_ulObject;            // for API
  unsigned long sm_ulProbeObject;       // for API
...
public:
  ...
  unsigned long GetShadowSize(void);
  // returns used memory - static, dynamic and uploaded size separately, slack space ratio (0-1 float) and flatness
  BOOL GetUsedMemory( long &slStaticSize, long &slDynamicSize, long &slUploadSize, FLOAT &fSlackRatio);
  ...
  unsigned long UpdateDynamicLayers(void);
