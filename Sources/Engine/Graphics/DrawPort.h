...
public:
  ...
  unsigned long dp_ulBlendingRA, dp_ulBlendingGA, dp_ulBlendingBA; // r*a, g*a, b*a
  unsigned long dp_ulBlendingA;
...
//interface:
  ...
  unsigned long GetTextWidth( const CTString &strText) const; 
  ...
  // draw line
  void DrawLine( PIX pixI0, PIX pixJ0, PIX pixI1, PIX pixJ1, COLOR col, unsigned long typ=_FULL_) const;
  void DrawLine3D( FLOAT3D v0, FLOAT3D v1, COLOR col) const; // in 3D
  // draw border
  void DrawBorder( PIX pixI, PIX pixJ, PIX pixWidth, PIX pixHeight, COLOR col, unsigned long typ=_FULL_) const;
