...
ENGINE_API extern unsigned long _ulMaxLengthOfSavingFile;
...
inline int CChunkID::operator==(const CChunkID &cidOther) const {
  return (*((unsigned long *)&cid_ID[0]) == *((unsigned long *)&cidOther.cid_ID[0]));
};
inline int CChunkID::operator!=(const CChunkID &cidOther) const {
  return (*((unsigned long *)&cid_ID[0]) != *((unsigned long *)&cidOther.cid_ID[0]));
};
...
public:
  ...
  long strm_slDictionaryPos; // dictionary position in file (0 for no dictionary)
  ...
  void ReadDictionary_intenal_t(long slOffset);
...
public:
  ...
  virtual void Read_t(void *pvBuffer, long slSize) = 0; // throw char *
  /* Write a block of data to stream. */
  virtual void Write_t(const void *pvBuffer, long slSize) = 0; // throw char *

  /* Seek in stream. */
  virtual void Seek_t(long slOffset, enum SeekDir sd) = 0; // throw char *
  /* Set absolute position in stream. */
  virtual void SetPos_t(long slPosition) = 0; // throw char *
  /* Get absolute position in stream. */
  virtual long GetPos_t(void) = 0; // throw char *
  /* Get size of stream */
  virtual long GetStreamSize(void) = 0;
  /* Get CRC32 of stream */
  virtual unsigned long GetStreamCRC32_t(void) = 0;
  ...
  inline CTStream &operator>>(unsigned long &ul)  { Read_t(&ul, sizeof(ul)); BYTESWAP(ul); return *this; } // throw char *
  inline CTStream &operator>>(long &sl)  { Read_t(&sl, sizeof(sl)); BYTESWAP(sl); return *this; } // throw char *
  ...
  inline CTStream &operator>>(uint64_t i) { Read_t( &i, sizeof( i)); BYTESWAP( i); return *this; } // throw char *
  ...
  inline CTStream &operator<<(unsigned long ul)  { BYTESWAP(ul); Write_t(&ul, sizeof(ul)); return *this; } // throw char *
  inline CTStream &operator<<(long sl)  { BYTESWAP(sl); Write_t(&sl, sizeof(sl)); return *this; } // throw char *
  ...
  inline CTStream &operator<<(uint64_t i) { BYTESWAP( i); Write_t( &i, sizeof( i)); return *this; } // throw char *
  ...
  virtual void GetLine_t(char *strBuffer, long slBufferSize, char cDelimiter='\n'); // throw char *
  ...
  virtual long GetSize_t(void); // throw char *
  virtual void ReadRawChunk_t(void *pvBuffer, long slSize); // throw char *
  virtual void ReadChunk_t(void *pvBuffer, long slExpectedSize); // throw char *
  virtual void ReadFullChunk_t(const CChunkID &cidExpected, void *pvBuffer, long slExpectedSize); // throw char *
  virtual void *ReadChunkAlloc_t(long slSize=0); // throw char *
  virtual void ReadStream_t(CTStream &strmOther); // throw char *

  virtual void WriteID_t(const CChunkID &cidSave); // throw char *
  virtual void WriteSize_t(long slSize); // throw char *
  virtual void WriteRawChunk_t(void *pvBuffer, long slSize); // throw char *  // doesn't write length
  virtual void WriteChunk_t(void *pvBuffer, long slSize); // throw char *
  virtual void WriteFullChunk_t(const CChunkID &cidSave, void *pvBuffer, long slSize); // throw char *
  ...
  void DictionaryWriteBegin_t(const CTFileName &fnmImportFrom, long slImportOffset);
  // stop writing a with a dictionary
  void DictionaryWriteEnd_t(void);
  // start reading a with a dictionary
  long DictionaryReadBegin_t(void);  // returns offset of dictionary for cross-file importing
...
private:
  ...
  long fstrm_slZipSize; // size of the zip-file entry
...
public:
  ...
  void Close(void);
  /* Get CRC32 of stream */
  unsigned long GetStreamCRC32_t(void);

  /* Read a block of data from stream. */
  void Read_t(void *pvBuffer, long slSize); // throw char *
  /* Write a block of data to stream. */
  void Write_t(const void *pvBuffer, long slSize); // throw char *

  /* Seek in stream. */
  void Seek_t(long slOffset, enum SeekDir sd); // throw char *
  /* Set absolute position in stream. */
  void SetPos_t(long slPosition); // throw char *
  /* Get absolute position in stream. */
  long GetPos_t(void); // throw char *
  /* Get size of stream */
  long GetStreamSize(void);
  /* Check if file position points to the EOF */
  BOOL AtEOF(void);
...
public:
  ...
  long mstrm_slLocation;    // location in the stream
...
public:
  ...
  CTMemoryStream(void *pvBuffer, long slSize, CTStream::OpenMode om = CTStream::OM_READ);
  /* Destructor. */
  virtual ~CTMemoryStream(void);

  /* Lock the buffer contents and it's size. */
  void LockBuffer(void **ppvBuffer, long *pslSize);
    /* Unlock buffer. */
  ...
  long GetPos_t(void); // throw char *
  /* Get size of stream. */
  long GetSize_t(void); // throw char *
  /* Get size of stream */
  long GetStreamSize(void);
  /* Get CRC32 of stream */
  unsigned long GetStreamCRC32_t(void);
...
// Get file timestamp
ENGINE_API long GetFileTimeStamp_t(const CTFileName &fnmFile); // throw char *
// Get CRC32 of a file
ENGINE_API unsigned long GetFileCRC32_t(const CTFileName &fnmFile); // throw char *
...
ENGINE_API INDEX ExpandFilePath(unsigned long ulType, const CTFileName &fnmFile, CTFileName &fnmExpanded);
...
ENGINE_API void MakeDirList(
  ...
  unsigned long ulFlags                 // additional flags
);
